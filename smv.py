...
for input_path in input_paths:
    # subprocess.run("rm -rf afacomp_simpl_smv/" + input_path, shell=True, check=True)
    subprocess.run("mkdir -p afacomp_simpl_smv/" + input_path, shell=True, check=True)
    ...
    for input_path2 in input_paths2:
        ...
        subprocess.run(
            "automata-safa-one prettyToSmv"
            f" < afacomp_push_pos_not/{input_path}/{input_path2}"
            f" > afacomp_simpl_smv/{input_path}/{input_path2[:-4]}.smv",
            shell=True,
            check=True,
        )
